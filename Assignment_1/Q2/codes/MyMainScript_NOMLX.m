...
%...
%where is the pixel intensity after linear contrast stretching,  and  are the minimum and maximum values of pixel intensity in the input image. For coloured images, this is applied to each channel independently.
tic;
...
toc;
%Linear contrast stretching is ineffective in improving contrast in the church.png image. Linear contrast stretching aims to stretch the range of the image to span the entire possible range i.e 0-255. However, the input image contains pixel intensities of both 0(white) and 255(black) in all 3 channels. Thus the transormation is an identity transform, with the output identical to the input image.
...
toc;
%Applying Historgram equaisation independently for each channel results in a significant change in colour (as made abundantly clear for the retina image). 
%A variation of the independent channel histogram equalisation is the average channel histogram equalisation, where the average transform across channels is applied to all channels. While the colour scheme is retained in this enhancement, for certain applications (such as a pre-processing step before segmenting the eye blood vessels), independent channel histogram equaisation maybe preferred.
%Comparing the performance of Histogram Equalisation and contrast stretching for image(5), it is clear that historgram equalisation is much better in improving contrast than linear contrast stretching. Linear contrast stretching's effects is greatly diminished if the range of pixel values is large, regardless of the number of such locations of extremely small or latge values. Histogram equalisation, on the other hand, has sufficient non-linearity to not be weakened by such factors.
...
my_imshow(HM_op, 'Histogram Matched Retina');
 toc;
%Simply applying histogram equalisation on mulit-channeled images (like RGB images), changes the colouring of the image (eg the retina is white not orange). This is an issue If we desired to only improve contrast, while maintaining the colouring, Histogram equilisation on the other hand is able to ensure the output image has better contrast without disturbing the colouring.
%Masking is critical to ensure the best enhancement, else the black background would result in a spike in the histogram and distort the transformation.
...
toc;
%The optimum window size for best contrast is dependent on the image. Clearly, the small window size amplifies noise a lot. In its extreme, the output resembles noise (eg. Applying AHE with window size=3 for canyon). For coloured images, smaller window sizes also distorts the colour scheme.
%On the flip side if the window size is too large, the transformation is simlar to that of vanilla Histogram Equalisation and contrast improvement is lower. This is explained by the fact that to improve contrast, local pixel intensity is of importanct, if the window is too large, the histogram obtained is less sensitive to the pixel intensties in the immediate neighbourhood of the pixel.
...
canyon_thresh = 0.05;
tic;
canyon_clahe_full = myCLAHE(canyon,421,canyon_thresh);
my_imshow((canyon_clahe_full)/255, cat(2,'Independent channel CL-Adaptive Histogram Equalised TEM, h=',num2str(canyon_thresh)));
toc;

tic;
canyon_clahe_half = myCLAHE(canyon,421,canyon_thresh/2);
my_imshow((canyon_clahe_half)/255, cat(2,'Independent channel CL-Adaptive Histogram Equalised TEM, h=',num2str(canyon_thresh/2)));
toc;

toc;
%The histogram-threshold parameter governs the behavior of the CLAHE transform. If h is very small, the output would be very similar to the input while if h is large, the output would match that of AHE, for the same window size.

%For better contrast, we desire to use smaller window sizes for AHE, however this also applifies noise. CLAHE enables us to use smaller windows, with lesser noise in the output. 
%This is best highlighted by the fact that for TEM, we had to resort to using a larger window size of 251, but CLAHE can give similar enhancement will a window size of just 51.
